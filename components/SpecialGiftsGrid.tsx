...
export const SpecialGiftsGrid: React.FC<SpecialGiftsGridProps> = ({ title, items }) => {
  return (
    <section className="max-w-[1560px] mx-auto px-5 md:px-16 lg:px-24 py-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        {items.map((item) => (
          <div key={item.id} className="flex flex-col sm:flex-row items-center sm:items-center gap-2 sm:gap-3 border border-gray-300 rounded-xl p-2 sm:pr-4 hover:shadow-md cursor-pointer transition-shadow bg-white sm:h-20 text-center sm:text-left">
